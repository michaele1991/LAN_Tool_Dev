...
class GBEImageCreator(tk.Tk):
    def __init__(self):
        ...
        self.gbe_folder_labels: dict[str, Path] = {}
    ...
    def _load_gbe_folders(self):
        """Scan GBE_Image for project folders that contain Excel workbooks."""
        ...
        for workbook in gbe_image_root.rglob("*"):
            if not workbook.is_file():
                continue
            if workbook.name.startswith("~$") or workbook.suffix.lower() not in workbook_suffixes:
                continue
            if any(part.startswith(".") for part in workbook.relative_to(gbe_image_root).parts):
                continue
            projects.setdefault(workbook.parent, workbook)

        self.gbe_folders = sorted(projects.keys(), key=lambda p: str(p.relative_to(gbe_image_root)).lower())
        self.gbe_folder_labels = {
            str(folder.relative_to(gbe_image_root)).replace("\\", "/"): folder
            for folder in self.gbe_folders
        }
        self.folder_combo["values"] = list(self.gbe_folder_labels.keys())
    ...
    def on_project_select(self):
        ...
        folder_path = self.gbe_folder_labels.get(folder_name)
        if folder_path is None:
            folder_path = next((f for f in self.gbe_folders if f.name == folder_name), None)
        ...
        # Find Excel workbook, preferring macro-enabled files for build projects.
        excel_files = sorted(
            [
                p for p in self.current_folder.iterdir()
                if p.is_file() and not p.name.startswith("~$") and p.suffix.lower() in {".xlsm", ".xlsx"}
            ],
            key=lambda p: (p.suffix.lower() != ".xlsm", p.name.lower()),
        )
        if excel_files:
            self.current_xlsm = excel_files[0]
